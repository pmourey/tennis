...
# ─────────────────────────────────────────────────────────────────────────────
#  MODULE TOURNOI INTERNE
# ─────────────────────────────────────────────────────────────────────────────

class Tournament(db.Model):
    """Tournoi interne ou open organisé par un club."""
    __tablename__ = 'tournament'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.String(100), nullable=False)
    club_id = db.Column(db.String(8), db.ForeignKey('club.id', ondelete='CASCADE'), nullable=False)
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
    is_open = db.Column(db.Boolean, default=False, nullable=False)  # True = Open, False = Interne
    surface = db.Column(db.String(20), nullable=False, default='TB')  # TB, Dur, Mixte
    status = db.Column(db.String(20), nullable=False, default='DRAFT')  # DRAFT, OPEN, IN_PROGRESS, CLOSED
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    notes = db.Column(db.Text, nullable=True)

    club = relationship('Club')
    categories = relationship('TournamentCategory', back_populates='tournament', cascade='all, delete-orphan')
    courts = relationship('TournamentCourt', back_populates='tournament', cascade='all, delete-orphan')

    @property
    def status_label(self):
        labels = {'DRAFT': 'Brouillon', 'OPEN': 'Inscriptions ouvertes',
                  'IN_PROGRESS': 'En cours', 'CLOSED': 'Terminé'}
...
class TournamentCategory(db.Model):
    """Catégorie de jeu dans un tournoi (ex: Hommes Seniors Simple)."""
    ...
    age_category_id = db.Column(db.Integer, db.ForeignKey('age_category.id'), nullable=True)
    gender = db.Column(db.Integer, nullable=False, default=0)  # 0=H, 1=F, 2=Mixte
    game_format = db.Column(db.Integer, nullable=False, default=1)  # 1=3 sets, 2=2 sets + super TB
    surfaces = db.Column(db.String(60), nullable=True, default='TB')  # ex: "TB" ou "TB,Dur" (surfaces autorisées)
    # draw_size calculé dynamiquement selon les inscrits
    min_ranking_id = db.Column(db.Integer, db.ForeignKey('ranking.id'), nullable=True)  # plancher : joueurs moins bien classés exclus
    max_ranking_id = db.Column(db.Integer, db.ForeignKey('ranking.id'), nullable=True)  # plafond : joueurs trop forts exclus
    allowed_series  = db.Column(db.String(20), nullable=True)  # séries autorisées ex: "3,4" ; null = toutes

    tournament = relationship('Tournament', back_populates='categories')
    age_category = relationship('AgeCategory')
    min_ranking = relationship('Ranking', foreign_keys=[min_ranking_id])
    max_ranking = relationship('Ranking', foreign_keys=[max_ranking_id])
    registrations = relationship('TournamentRegistration', back_populates='category', cascade='all, delete-orphan')
    draws = relationship('TournamentDraw', back_populates='category', cascade='all, delete-orphan')

    @property
    def surfaces_list(self) -> list:
        if not self.surfaces:
            return ['TB']
        return [s.strip() for s in self.surfaces.split(',') if s.strip()]

    @property
    def surfaces_label(self) -> str:
        names = {'TB': 'Terre battue', 'Dur': 'Dur', 'Gazon': 'Gazon', 'Synthétique': 'Synthétique'}
        return ', '.join(names.get(s, s) for s in self.surfaces_list)

    @property
    def allowed_series_list(self) -> list[int]:
        """Retourne la liste des numéros de série autorisés (vide = toutes)."""
        if not self.allowed_series:
            return []
        return [int(s) for s in self.allowed_series.split(',') if s.strip().isdigit()]

    @property
    def name(self):
        gender_str = {0: 'Hommes', 1: 'Femmes', 2: 'Mixte'}.get(self.gender, '')
        cat_str = self.age_category.name if self.age_category else 'Seniors'
        fmt_str = f'Format {self.game_format}'
        return f'{gender_str} {cat_str} – {fmt_str}'

    @property
    def format_label(self):
        if self.game_format == 1:
            return '3 sets (6 jeux, TB à 6-6)'
        return '2 sets + super TB (10 pts)'

    @property
    def match_duration_minutes(self):
        return 120 if self.game_format == 1 else 90

    @property
    def draw_size(self):
        n = len([r for r in self.registrations if r.status != 'WITHDRAWN'])
        if n <= 4: return 4
        elif n <= 8: return 8
        elif n <= 16: return 16
        elif n <= 32: return 32
...
class TournamentCourt(db.Model):
    """Terrain disponible pour un tournoi."""
    __tablename__ = 'tournament_court'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    tournament_id = db.Column(db.Integer, db.ForeignKey('tournament.id', ondelete='CASCADE'), nullable=False)
    name = db.Column(db.String(50), nullable=False)
    surface = db.Column(db.String(20), nullable=True)
...
class TournamentRegistration(db.Model):
    """Inscription d'un joueur dans une catégorie de tournoi."""
    __tablename__ = 'tournament_registration'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    tournament_id = db.Column(db.Integer, db.ForeignKey('tournament.id', ondelete='CASCADE'), nullable=False)
    category_id = db.Column(db.Integer, db.ForeignKey('tournament_category.id', ondelete='CASCADE'), nullable=False)
    ...
    status = db.Column(db.String(20), nullable=False, default='REGISTERED')  # REGISTERED, WITHDRAWN, QUALIFIED

    category = relationship('TournamentCategory', back_populates='registrations')
    player = relationship('Player')
    availabilities = relationship('TournamentAvailability', back_populates='registration', cascade='all, delete-orphan')
...
class TournamentAvailability(db.Model):
    """Créneau de disponibilité d'un joueur pour un tournoi."""
    __tablename__ = 'tournament_availability'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    registration_id = db.Column(db.Integer, db.ForeignKey('tournament_registration.id', ondelete='CASCADE'), nullable=False)
    day_type = db.Column(db.String(10), nullable=False)  # 'weekday' ou 'weekend'
    time_start = db.Column(db.String(5), nullable=False)  # HH:MM
    time_end = db.Column(db.String(5), nullable=False)    # HH:MM
    notes = db.Column(db.String(100), nullable=True)

    registration = relationship('TournamentRegistration', back_populates='availabilities')


class TournamentDraw(db.Model):
    """Tableau d'une catégorie (qualifications ou tableau principal)."""
    __tablename__ = 'tournament_draw'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    category_id = db.Column(db.Integer, db.ForeignKey('tournament_category.id', ondelete='CASCADE'), nullable=False)
    draw_type = db.Column(db.String(20), nullable=False, default='MAIN')  # QUALIFYING ou MAIN
    name = db.Column(db.String(120), nullable=True)   # ex: "Qualifications NC–30/5"
    generated_at = db.Column(db.DateTime, default=datetime.utcnow)
    # Lien explicite vers le slot du tableau final (pour les tableaux qualificatifs)
    main_draw_match_id = db.Column(db.Integer, nullable=True)   # id du match cible dans le tableau final
    main_draw_slot = db.Column(db.String(2), nullable=True)     # 'p1' ou 'p2'
    qualif_number = db.Column(db.Integer, nullable=True)        # numéro du qualifié (Q1, Q2…)

    category = relationship('TournamentCategory', back_populates='draws')
    matches = relationship('TournamentMatch', back_populates='draw', cascade='all, delete-orphan')

    @property
    def display_name(self) -> str:
        if self.name:
            return self.name
        return 'Qualifications' if self.draw_type == 'QUALIFYING' else 'Tableau Final'

    def __repr__(self):
        return f'Tableau {self.display_name}'


class TournamentMatch(db.Model):
    """Match dans un tableau de tournoi."""
    __tablename__ = 'tournament_match'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    draw_id = db.Column(db.Integer, db.ForeignKey('tournament_draw.id', ondelete='CASCADE'), nullable=False)
    round_number = db.Column(db.Integer, nullable=False)   # 1=1er tour, 2=2e tour, etc.
    position = db.Column(db.Integer, nullable=False)        # position dans le tableau (1..draw_size/2)
    player1_id = db.Column(db.Integer, db.ForeignKey('player.id', ondelete='SET NULL'), nullable=True)
    player2_id = db.Column(db.Integer, db.ForeignKey('player.id', ondelete='SET NULL'), nullable=True)
    winner_id = db.Column(db.Integer, db.ForeignKey('player.id', ondelete='SET NULL'), nullable=True)
    court_id = db.Column(db.Integer, db.ForeignKey('tournament_court.id', ondelete='SET NULL'), nullable=True)
    scheduled_at = db.Column(db.DateTime, nullable=True)
    status = db.Column(db.String(20), nullable=False, default='PENDING')  # PENDING, SCHEDULED, COMPLETED, BYE, WALKOVER
    # Score sous forme textuelle (ex: "6/3 6/4" ou "6/3 3/6 10/7")
    score_text = db.Column(db.String(50), nullable=True)
    next_match_date_options = db.Column(db.String(200), nullable=True)  # Dates proposées pour la convocation

    draw = relationship('TournamentDraw', back_populates='matches')
    player1 = relationship('Player', foreign_keys=[player1_id])
    player2 = relationship('Player', foreign_keys=[player2_id])
    winner = relationship('Player', foreign_keys=[winner_id])
    court = relationship('TournamentCourt')

    @property
    def round_name(self):
        draw_size = self.draw.category.draw_size
        total_rounds = draw_size.bit_length() - 1
        rounds_left = total_rounds - self.round_number
        names = {0: 'Finale', 1: 'Demi-finale', 2: 'Quart de finale',
                 3: '8e de finale', 4: '16e de finale', 5: '32e de finale'}
        return names.get(rounds_left, f'Tour {self.round_number}')

    @property
    def display_score(self):
        """Score affiché avec les jeux du gagnant toujours à gauche."""
        if not self.score_text:
            return ''
        # Si pas de gagnant ou gagnant = player1, afficher tel quel
        if not self.winner_id or self.winner_id == self.player1_id:
            return self.score_text
        import re
        def swap_set(s):
            # Format normal: "6/7(2)" → "7(2)/6"
            m = re.match(r'^(\d+)/((\d+)(\(\d+\))?)$', s)
            if m:
                return m.group(2) + '/' + m.group(1)
            # Format déjà inversé par erreur: "7(2)/6" → "6/7(2)"
            m2 = re.match(r'^(\d+)(\(\d+\))/(\d+)$', s)
            if m2:
                return m2.group(3) + '/' + m2.group(1) + m2.group(2)
            # Format simple: "6/1" → "1/6"
            m3 = re.match(r'^(\d+)/(\d+)$', s)
            if m3:
                return m3.group(2) + '/' + m3.group(1)
            return s
        return ' '.join(swap_set(part) for part in self.score_text.split())
